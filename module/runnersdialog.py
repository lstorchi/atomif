...
class runmifprofilesdialog(QtWidgets.QDialog):
    ...
    def __init__(self, parent=None):
        ...
        self.grid.addWidget(probe_label, 3, 0)
        self.grid.addWidget(self.probe_line, 3, 1)

        self.grid.addWidget(minima_label, 4, 0)
        self.grid.addWidget(self.minima_line, 4, 1)

        self.grid.addWidget(self.kontcheckbox, 5, 1)

        self.grid.addWidget(self.okbutton, 6, 2)

    def run(self):

        self.close()

class runmifinteractiondialog(QtWidgets.QDialog):

    def __init__(self, parent=None):

        super(runmifinteractiondialog, self).__init__(parent)
